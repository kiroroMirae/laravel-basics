...
@section('content')
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('category.store', ['user' => auth()->user()->id]) }}" method="POST" class="bg-white w-6/12 overflow-hidden shadow-sm sm:rounded-lg px-[20px] py-[16px] ">
                @csrf
                ...
                <div class="flex flex-col gap-y-[15px]">
                    <div class="flex justify-between items-center">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" id="" class="w-9/12 rounded-[5px]">
                    </div>
                    @error('name')
